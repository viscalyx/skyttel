...
import { alex, robin, createInstallation } from '../support/installation.js';
...
test('two installations honor different administrators and deny another identity the bootstrap', async () => {
  ...
  try {
    ...
    expect(await (await client.get(`${second.origin}/api/bootstrap`)).json()).toMatchObject({ status: 'forbidden' });
    ...
    expect((await client.get(`${second.origin}/api/households/${firstHousehold.id}`)).status()).toBe(403);
    expect((await client.get(`${first.origin}/api/households/${firstHousehold.id}`)).status()).toBe(401);
  } finally {
...
test('a second identity with the same email cannot acquire the first identity or its membership', async () => {
  ...
  try {
    ...
    expect((await outsider.get(`${installation.origin}/api/households/${household.id}`)).status()).toBe(403);
...
test('bootstrap is atomic and rejects cross-origin and malformed submissions', async () => {
  ...
  try {
    ...
    expect((await client.post(`${installation.origin}/api/households`, {
      headers: { origin: 'https://attacker.example' }, data: { name: 'Intrång' },
    })).status()).toBe(403);
    ...
    expect(await (await client.get(`${installation.origin}/api/bootstrap`)).json())
      .toMatchObject({ status: 'ready', household: winner.household });
...
test('current membership isolates households and revocation defeats an existing session', async () => {
  ...
  try {
    await signIn(administrator, installation.origin);
    const { household } = await (await createHousehold(administrator, installation.origin)).json();
    const adminState = await (await administrator.get(`${installation.origin}/api/bootstrap`)).json();
    installation.setIdentity({ ...robin, name: alex.name });
    await signIn(otherMember, installation.origin, 'microsoft');
    const outsiderState = await (await otherMember.get(`${installation.origin}/api/bootstrap`)).json();
    expect(outsiderState.status).toBe('forbidden');
    installation.seedMembership(outsiderState.user.id, 'another-household', 'Hushållet Eken');
    expect(await (await otherMember.get(`${installation.origin}/api/households/another-household`)).json())
      .toMatchObject({ household: { name: 'Hushållet Eken', role: 'member' } });
    const forbidden = await otherMember.get(`${installation.origin}/api/households/${household.id}`);
    expect(forbidden.status()).toBe(403);
    expect(await forbidden.text()).not.toContain(household.name);
    expect((await administrator.get(`${installation.origin}/api/households/another-household`)).status()).toBe(403);
    installation.revokeMembership(adminState.user.id);
    expect((await administrator.get(`${installation.origin}/api/households/${household.id}`)).status()).toBe(403);
    expect(await (await administrator.get(`${installation.origin}/api/bootstrap`)).json()).toMatchObject({ status: 'forbidden' });
...
test('forged OAuth callbacks and unavailable authentication routes do not grant access', async () => {
  ...
  try {
    ...
    expect(await (await client.get(`${installation.origin}/api/bootstrap`)).json()).toMatchObject({ status: 'anonymous' });
    for (const path of ['link-social', 'unlink-account', 'get-access-token', 'sign-up/email', 'test-login']) {
      const result = await client.post(`${installation.origin}/api/auth/${path}`, {
        headers: { origin: installation.origin }, data: { provider: 'google' },
      });
