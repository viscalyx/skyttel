...
const variants = {
  A: {
    name: 'Ett verktyg i taget',
    description: 'En flytande arbetsyta. Tillbaka leder genom arbetet.',
  },
  B: {
    name: 'Samtal bredvid',
    description: 'Samtal och arbete sida vid sida. På mobil växlar du mellan dem.',
  },
  C: { name: 'Arbetsflikar', description: 'En större arbetsyta med öppna verktyg som flikar.' },
};
...
export function NavigationPrototype() {
  ...
  const [role, setRole] = useState<'member' | 'administrator' | 'operator'>('administrator');
  ...
  const permitted = (!restricted.includes(page) || administrator) && (page !== 'costs' || operator);
  ...
  const showConversation = variant === 'B' && conversationOpen && ready;
  const mainPage = page === 'conversation' && variant === 'B' ? 'map' : page;
  const personalPage = ['login-methods', 'invitations', 'costs'].includes(mainPage);
  const showPanel = mainPage !== 'map' && (ready || (scenario === 'access' && personalPage));
  ...
  useLayoutEffect(() => {
    ...
    const surfaces = [
      ...root.querySelectorAll<HTMLElement>(
        '.np-toolbar, .np-status, .np-panel, .np-lab, .np-map-controls',
      ),
    ];
  ...
  useEffect(() => {
    ...
    const dismiss = (event: PointerEvent) => {
      const target = event.target as HTMLElement;
      if (target.closest('.np-panel, .np-toolbar, .np-lab, .np-status, .vp-map-node')) return;
      closePanel();
    };
  ...
  const panelHidden = showConversation && mobileConversation && compact;
    ...
    >
      ...
      <div className="np-shell">
        <header className="np-top">
          <nav className="np-toolbar" id="np-tools" aria-label="Skyttels verktyg">
            <span className="np-logo">
              <img src={smallLogo} alt="Skyttel" />
              <span className="np-tool-label">skyttel.</span>
            </span>
            {ready && (
              <>
                <button
                  className={`np-tool${voice ? ' np-active' : ' np-primary'}`}
                  type="button"
                  onClick={toggleVoice}
                ...
                >
                  {voice ? (
                    <svg className="vp-icon" viewBox="0 0 24 24" aria-hidden="true">
                      <circle
                        cx="12"
                        cy="12"
                        r="10"
                  ...
                  <span className="np-tool-label">{voice ? 'Stoppa tal' : 'Börja prata'}</span>
                </button>
                <button
                  className="np-tool"
                  data-tool="conversation"
                  type="button"
                  onClick={() => go('conversation')}
                  aria-label="Samtal och text"
                  title="Samtal och text"
                >
                  <PrototypeIcon name="text" />
                  <span className="np-tool-label">Samtal och text</span>
                </button>
                <button
                  className="np-tool"
                  data-tool="list"
                  type="button"
                  onClick={() => go('list')}
                  aria-label="Objekt och samband"
                  title="Objekt och samband"
                >
                  <PrototypeIcon name="list" />
                  <span className="np-tool-label">Objekt och samband</span>
                </button>
                <button
                  className="np-tool"
                  data-tool="more"
                ...
                >
                  ...
                  <span className="np-tool-label">Fler verktyg</span>
                </button>
                <button
                  className="np-tool"
                  type="button"
                  ...
                  aria-label={expanded ? 'Dölj verktygsnamn' : 'Visa verktygsnamn'}
                  title={expanded ? 'Dölj verktygsnamn' : 'Visa verktygsnamn'}
                >
                  <PrototypeIcon name={expanded ? 'back' : 'arrow'} />
                  <span className="np-tool-label">Dölj namn</span>
                </button>
              </>
            )}
          </nav>
          <div className="np-context">
            <strong>{ready ? 'Hushållet Lind' : 'Skyttel'}</strong>
            <span>{ready ? 'Gemensam karta · din personliga vy' : 'Skyttel'}</span>
          </div>
        </header>
        <section className="np-status" aria-label="Aktuell status">
          <div className="np-status-copy" role="status" aria-live="polite">
            <PrototypeIcon
              name={
                scenario === 'network' || saveState === 'failed' ? 'alert' : voice ? 'mic' : 'check'
              }
            />
            <span>{status}</span>
            {voice && (
              <span className="np-wave" aria-hidden="true">
                ▂ ▅ ▃ ▆ ▂
              </span>
            )}
          </div>
        ...
        <main className="np-workspace" id="np-work" tabIndex={-1} ref={workspaceRef}>
          {ready && variant === 'C' && page === 'map' && tabs.length > 0 && (
          ...
          {ready && !showPanel && !showConversation && (
          ...
          {showPanel && (
            <div className="np-panel-host" hidden={panelHidden}>
              <Panel
                ...
                className={anchor && mainPage === 'detail' ? 'np-anchored-detail' : ''}
                style={anchor && mainPage === 'detail' ? popupStyle : undefined}
                extra={
                  variant === 'C' && (
          ...
          {showConversation && (
            <div className="np-conversation-host" hidden={compact && !mobileConversation}>
              <Panel
                page="conversation"
        ...
        <aside className="np-lab" aria-label="Prototypens provverktyg">
          <div className="np-lab-row">
            ...
            <button type="button" onClick={() => switchVariant(1)} aria-label="Nästa alternativ">
              →
            </button>
            <button
              type="button"
          ...
          {saveState === 'pending' && (
            <div className="np-lab-row">
              <span>Sparande är simulerat. Välj svaret:</span>
