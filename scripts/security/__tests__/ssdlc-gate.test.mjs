...
describe('SSDLC gate', () => {
  ...
  it('requires evidence for Skyttel application, configuration, deployment, and validation paths', () => {
    const paths = [
      ...
      '.codex/config.toml',
      '.codex/rules/github.rules',
      'docs/development/devcontainer.md',
      'docs/development/testing.md',
      'docs/operations/security-checks.md',
      'scripts/check-container.mjs',
      'scripts/security/ssdlc-gate.mjs',
      'tests/integration/access.spec.ts',
    ...
    assert.deepEqual(
      classifyChangedFiles(['src/server/auth.ts', 'src/server/auth.ts']).find(
        (group) => group.id === 'authentication-authorization',
      ).files,
      ['src/server/auth.ts'],
    );
  });
  ...
  it('requires one completed checkbox with the exact marker', () => {
    ...
    assert.equal(
      checkboxState(
        completePrBody.replace('requirements -->', 'requirements-extra -->'),
        'requirements',
      ),
      'missing',
    );
    ...
    for (const prBody of [
      '',
      completePrBody.replace('[x]', '[ ]'),
      completePrBody + completePrBody,
    ]) {
  ...
  it('classifies the former sensitive path when a file is renamed into documentation', async () => {
    const { fetchImpl } = fakeGitHub({
      pages: [
        [
          {
            filename: 'docs/old-auth.md',
            previous_filename: 'src/server/auth.ts',
            status: 'renamed',
          },
        ],
      ],
  ...
  it('rejects missing credentials, invalid identities, API failures, and aborted requests', async () => {
    ...
    for (const overrides of [
      ...
      await assert.rejects(
        readPullRequestFromGitHub({ ...githubInput, ...overrides, fetchImpl: noFetch }),
      );
    }
    await assert.rejects(
      readPullRequestFromGitHub({
  ...
  it('runs local CLI inputs and reports pass, evidence failure, and argument errors', async () => {
    ...
    const fsImpl = {
      readFileSync: (path) => (path === 'changed.txt' ? 'src/server/auth.ts\n' : completePrBody),
    };
    assert.equal(
      await main(['--changed-files', 'changed.txt', '--pr-body', 'body.md'], {
        consoleObj,
        fsImpl,
      }),
      0,
    ...
    assert.throws(
      () => parseArgs(['--github-pr', '66', '--pr-body', 'body.md']),
  ...
  it('returns a failing exit code for incomplete GitHub evidence or API data', async () => {
    const { output, consoleObj } = capturedConsole();
    const env = {
      GITHUB_REPOSITORY: githubInput.repository,
      GITHUB_TOKEN: githubInput.token,
      PR_NUMBER: '66',
    };
